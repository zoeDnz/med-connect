...
class GerarDescricaoAnuncioView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"erro": f"Registro não encontrado: {str(e)}"},
                status=status.HTTP_404_NOT_FOUND
            )

        validade_formatada = (
            lote.dt_validade.strftime('%d/%m/%Y')
            if hasattr(lote.dt_validade, 'strftime')
            else lote.dt_validade
        )
        ...
        - Enfatize que o item está disponível para repasse.
        - Não restrinja o motivo apenas a "sobra de procedimento".
        ...
        try:
            ...
            return Response({"texto_sugerido": response_gemini.text}, status=status.HTTP_200_OK)
